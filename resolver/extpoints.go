...
type hostResolverConfigExt struct {
	*extensionPoint
}

func (ep *hostResolverConfigExt) Unregister(name string) bool {
	return ep.unregister(name)
}

func (ep *hostResolverConfigExt) Register(extension HostResolverConfig, name string) bool {
	return ep.register(extension, name)
}

func (ep *hostResolverConfigExt) Lookup(name string) HostResolverConfig {
	ext := ep.lookup(name)
	if ext == nil {
		return nil
	}
	return ext.(HostResolverConfig)
}

func (ep *hostResolverConfigExt) Select(names []string) []HostResolverConfig {
	var selected []HostResolverConfig
...
func (ep *hostResolverConfigExt) All() map[string]HostResolverConfig {
	all := make(map[string]HostResolverConfig)
	for k, v := range ep.all() {
		all[k] = v.(HostResolverConfig)
	}
	return all
}

func (ep *hostResolverConfigExt) Names() []string {
	var names []string
	for k := range ep.all() {
